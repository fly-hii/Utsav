...
export default function ForgotPasswordScreen() {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      setLoading(true);
      await CommitteeAuthService.sendOtp(phone, 'FORGOT_PASSWORD');
  ...
  const handleResetPassword = async () => {
    ...
    try {
      setLoading(true);
      await CommitteeAuthService.resetPasswordWithOtp(phone, otp, newPassword);
            ...
            {step === 1 ? (
              <>
                <View style={styles.inputGroup}>
                  ...
                  <View style={[styles.inputWrapper, { backgroundColor: isDark ? 'rgba(0,0,0,0.2)' : 'rgba(255,255,255,0.3)', borderColor: colors.glassBorder }]}>
                ...
                >
                  <LinearGradient colors={GRADIENTS.gold} style={styles.buttonGradient}>
                    {loading ? <ActivityIndicator color={colors.textPrimary} /> : <Text style={[styles.buttonText, { color: colors.cream }]}>Get OTP</Text>}
                  </LinearGradient>
                </TouchableOpacity>
              </>
            ) : (
              <>
                <View style={styles.inputGroup}>
                  ...
                  <TextInput
                    style={[styles.input, styles.otpInput, { backgroundColor: isDark ? 'rgba(0,0,0,0.2)' : 'rgba(255,255,255,0.3)', borderColor: colors.glassBorder, color: colors.textPrimary }]}
                    placeholder="• • • • • •"
                    placeholderTextColor={colors.textSecondary}
                ...
                <View style={styles.inputGroup}>
                  ...
                  <View style={[styles.passwordWrapper, { backgroundColor: isDark ? 'rgba(0,0,0,0.2)' : 'rgba(255,255,255,0.3)', borderColor: colors.glassBorder }]}>
                    <TextInput
...
const styles = StyleSheet.create({
  ...
  inputWrapper: { flexDirection: 'row', alignItems: 'center', borderRadius: 12, borderWidth: 1, paddingHorizontal: 16 },
  prefix: { fontSize: 16, fontWeight: '600', marginRight: 10 },
  input: { flex: 1, height: 50, fontSize: 16 },
  otpInput: { borderRadius: 12, borderWidth: 1, height: 56, fontSize: 24, letterSpacing: 8, fontWeight: '700' },
  passwordWrapper: { flexDirection: 'row', alignItems: 'center', borderRadius: 12, borderWidth: 1, paddingHorizontal: 16 },
